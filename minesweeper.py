...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        new_sent = Sentence(unknown_cells, count-num_mines)
        ...
        for row in range(cell[0] - 1, cell[0] + 2):
            for col in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= row < self.height and 0 <= col < self.width:
                    if (row, col) not in self.moves_made and (row, col) not in self.safes:
                        neighbors.add((row, col))
        new_sent = Sentence(neighbors, count)
        self.knowledge.append(new_sent)
        print(f"New sentence added:", new_sent)

        """for sent in self.knowledge:
            if new_sent.cells.issubset(sent.cells) and sent.count > 0 and new_sent.count > 0 and new_sent != sent:
                new_subset = sent.cells.difference(new_sent.cells)
                updated_sent = Sentence(new_subset, sent.count - new_sent.count)
                self.knowledge.append(updated_sent)"""
        ...
        print(f"Known Safe Moves", self.safes - self.moves_made)
    ...
    def infer_knowledge(self):
        ...
        for sentence1 in knowledgecopy:
